import Image from 'next/image';
import React from 'react';
import idxpartnersImg from '../public/assets/projects/idxpartners.jpg';
import { RiRadioButtonFill } from 'react-icons/ri';
import Link from 'next/link';

const idxpartners = () => {
  return (
    <div className='w-full'>
      <div className='w-screen h-[50vh] relative'>
        <div className='absolute top-0 left-0 w-full h-[50vh] bg-black/70 z-10' />
        <Image
          className='absolute z-1'
          layout='fill'
          objectFit='cover'
          src={idxpartnersImg}
          alt='/'
        />
        <div className='absolute top-[70%] max-w-[1240px] w-full left-[50%] right-[50%] translate-x-[-50%] translate-y-[-50%] text-white z-10 p-2'>
          <h2 className='py-2'>Virtual Internship Experience (id/x partners)</h2>
          <h3>Python | Jupyter Notebook | Machine Learning</h3>
        </div>
      </div>

      <div className='max-w-[1240px] mx-auto p-2 grid md:grid-cols-5 gap-8 py-8'>
        <div className='col-span-4'>
          <p>Project</p>
          <h2>Overview</h2>
          <p className='text-justify'>
          This is a final project of one month virtual internship provided by id/x partners and Rakamin Academy. Build a credit scoring model using machine learning for loan company. The dataset is provided by id/x partners. The model is built using Python and Jupyter Notebook. The model is evaluated using F1 Score and AUC Score. 
          </p>
          <a
            href='https://github.com/junkevin88/project_idxpartners'
            target='_blank'
            rel='noreferrer'
          >
            <button className='px-8 py-2 mt-4 mr-8'>Code</button>
          </a>
        </div>
        <div className='col-span-4 md:col-span-1 shadow-xl shadow-gray-100 rounded-xl py-4'>
          <div className='p-2'>
            <p className='text-center font-bold pb-2'>Technologies</p>
            <div className='grid grid-cols-3 md:grid-cols-1'>
              <p className='text-gray-100 py-2 flex items-center'>
                <RiRadioButtonFill className='pr-1' /> Python
              </p>
              <p className='text-gray-100 py-2 flex items-center'>
                <RiRadioButtonFill className='pr-1' /> Jupyter Notebook
              </p>
              <p className='text-gray-100 py-2 flex items-center'>
                <RiRadioButtonFill className='pr-1' /> ML (Supervised Learning)
              </p>
              <p className='text-gray-100 py-2 flex items-center'>
                <RiRadioButtonFill className='pr-1' /> Exploratory Data Analysis
              </p>
              <p className='text-gray-100 py-2 flex items-center'>
                <RiRadioButtonFill className='pr-1' /> XGBoost
              </p>
            </div>
          </div>
        </div>
        <Link href='/#projects'>
          <p className='underline cursor-pointer'>Back</p>
        </Link>
      </div>
    </div>
  );
};

export default idxpartners;
